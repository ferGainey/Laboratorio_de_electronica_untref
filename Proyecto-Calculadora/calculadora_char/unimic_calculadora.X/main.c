...
char array_lcd_izq[6];
char array_lcd_der[6];
char* operador = "";
...
int main(void) {
    ...
    while (1) {
        ...
        if (column3 == 1) {
            //write_in_lcd(6); // TODO: definir =
            if (operador == "+") {           

                char num_izq = "";
                for (int i = 0; i < contador_numeros_ingresados_izq; i++) {
                    num_izq = num_izq + array_lcd_izq[i];
                };

                char num_der = "";
                for (int j = 0; j < contador_numeros_ingresados_der; j++) {
                    num_der = num_der + array_lcd_der[j];
                };

                lcd_gotoxy(1, 2);
                write_in_lcd(num_izq);
            };
            while (column3 == 1) {
            };
        };
